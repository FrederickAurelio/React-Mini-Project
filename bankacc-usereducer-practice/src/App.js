...
1. Let's implement a simple bank account! It's similar to the example that
 I used as an analogy to explain how useReducer works, but it's simplified 
 (we're not using account numbers here)

2. Use a reducer to model the following state transitions: openAccount, deposit,
 withdraw, requestLoan, payLoan, closeAccount. Use the `initialState` below to get started.
...
deposit amount of 500 to open an account (which means that the balance will start at 500)
...
7. Customer can only close an account if there is no loan, AND if the balance
 is zero. If this condition is not met, just return the state. If the condition 
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button onClick={() => { dispatch({ type: "openAcc" }) }} disabled={isActive}>
          Open account
        </button>
      </p>
      <p>
